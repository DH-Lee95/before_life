...
- 기본 입력의 성별은 필수이며 결정적 결과 키에 포함한다. 생성된 대표 전생과 AI 서사에도 전생에서의 성별을 명시한다.
- 시대와 지역은 서로 검증된 `pastLifeWorlds` 조합에서 함께 선택한다.
- 깊은 기록은 사랑, 마지막 날, 재산과 신분, 결정적 선택, 미완의 약속, 현생의 흔적, 부모와 자식의 인연 총 7개다.
- 반복 주제 답변에 가장 가까운 기록 1개만 무료 전체 본문으로 제공한다.
- 나머지 6개는 제목, 예상 독서 시간, 개인화 미리보기만 무료 응답에 포함한다. 잠긴 본문은 클라이언트에 전송하지 않는다.
- 각 깊은 기록은 같은 대표 전생에서 파생되는 장면별 외전이며 1소울로 연다.
- 부모와 자식의 인연은 전생의 부모·자식 관계에서 반복된 선택을 현생의 가족 관계 패턴과 연결하되, 특정 현생 인물이 전생의 누구라고 단정하지 않는다.
...
- 소울 상품은 1개 1,000원, 3개 2,490원, 5개 3,990원, 전체 서랍용 7개 4,990원으로 config에서 관리한다.
