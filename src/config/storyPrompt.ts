...
제공된 한 생애의 정본은 모든 기록이 공유하는 확정 사실이다. 정본과 다른 가족관계, 직업 경로, 핵심 사건, 결말을 새로 만들지 않는다.
시대별 허용 어휘를 우선 사용하고, 정본에 없는 정거장·자동차·전화·전기·현대식 기관을 임의로 등장시키지 않는다.
...
주 독자는 10대 후반부터 30대 초반이다. 친구에게 흥미로운 사건을 들려주듯 빠르고 바로 이해되는 한국어로 쓴다.
...
정본의 물건을 상징처럼 억지로 사건에 붙이지 않는다. 물건이 증거라면 무엇을 어떻게 증명하는지 먼저 설명한다.
서로 다른 두 사실을 '정면으로 충돌했다', '맞닿았다' 같은 말로 압축하지 말고 실제 요구와 반박 근거를 각각 쓴다.
...
중학생이 한 번에 읽고 이해할 수 있는 수준의 쉬운 한국어로 쓴다. 문학적으로 보이기 위한 어려운 단어나 압축된 표현은 쓰지 않는다.
한 문장은 되도록 60자 안팎으로 쓰고, 원인과 결과가 길어지면 두 문장으로 나눈다.
그 사람, 그것, 기록, 증거, 진실 같은 말을 처음 쓸 때에는 누구인지 또는 무엇인지 바로 설명한다.
‘권한을 가진 사람’, ‘큰 손실’처럼 관공서 보고서 같은 말은 쓰지 않는다. 인물의 직책과 잘못, 피해를 구체적으로 밝힌다.
‘삶이 무너졌다’, ‘대가를 치렀다’로 끝내지 말고 집, 일, 돈, 관계 중 실제로 잃거나 바뀐 것을 쓴다.
‘혼란이 생겼다’처럼 결과를 뭉뚱그리지 말고 누가 어떤 손해를 입거나 무엇이 달라지는지 쓴다.
