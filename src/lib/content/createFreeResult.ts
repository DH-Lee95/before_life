...
export function createFreeResult(profile: SoulProfile): FreeResultContent {
  ...
  return {
    ...
    summary: `당신의 대표 전생은 ${record.period} ${record.region}의 ${record.location}에서 ${genderLabel(record.gender)}으로, ${asRole(record.occupation)} 살아간 사람의 기록입니다.`,
...
function createDeepDive(
  ...
  switch (contentType) {
    ...
    case "last_day":
      return {
        opening: `${record.period} ${record.region}의 ${record.location}. 밤새 이어진 고열 끝에 의원은 오늘 밤을 넘기기 어렵다고 말했습니다. ${asPastRole(record.occupation)} 당신은 남은 시간을 일터 정리에 쓰지 않고, 오래전 자신이 쫓아낸 사람을 불러 숨겨온 진실을 직접 말하기로 했습니다.`,
        chapters: [
          { title: "마지막으로 부른 이름", paragraphs: [`당신은 남은 시간을 확인한 뒤 만나야 할 단 한 사람의 이름을 적었습니다. 두려움을 크게 말하는 대신 상황을 통제하려는 것이 당신다운 방식이었지만, 종이를 건네는 손만은 멈추지 않고 떨렸습니다.`, `해가 중천에 올랐을 때 그 사람이 문 앞에 나타났습니다. 그는 용서하러 온 것이 아니라 왜 자신에게 모든 책임을 떠넘겼는지 듣기 위해 왔다고 말했습니다. 당신은 평생 피했던 질문에 그날 안으로 답해야 했습니다.`] },
          { title: "보호라는 이름의 거짓말", paragraphs: [`오래전 권한을 가진 사람의 잘못으로 큰 손실이 생겼을 때, 당신은 여러 가족의 생계를 지키겠다는 이유로 힘없는 그 사람에게 책임을 돌렸습니다. 많은 사람은 안전했지만 한 사람의 삶은 무너졌고, 당신은 그것을 어쩔 수 없는 선택이라고 합리화했습니다.`, `당신이 모든 사실을 고백하자 그는 용서할 수 없다고 말했습니다. 그러나 자신이 쫓겨난 진짜 이유를 알게 된 것만으로도 빼앗겼던 삶의 일부를 되찾았다고 했습니다. 당신은 용서를 얻기 위해서가 아니라 대가를 제자리로 돌려놓기 위해 진실을 남겼습니다.`] },
          { title: "해가 지기 전의 선택", paragraphs: [`진실이 알려지면 잘못을 저지른 권력자는 자리를 잃게 됩니다. 그에게 의지해 살던 사람들도 당분간 생계가 어려워질 수 있었습니다. 하지만 당신은 죄 없는 한 사람에게 책임을 떠넘긴 채 죽을 수는 없었습니다. 당신은 당시 상황을 적은 편지와 문서를 누명을 쓴 사람에게 건넸습니다. 세상에 공개할지는 그가 직접 정하게 했습니다.`, `그는 끝내 용서한다는 말은 하지 않았지만 해가 질 때까지 곁에 남았습니다. 기록은 죽음의 순간이 아니라, 평생 다른 사람을 위해 결정하던 사람이 처음으로 자신의 잘못을 직접 책임진 장면에서 멈춥니다.`] },
        ],
